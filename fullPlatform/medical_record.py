...
from printReports import Ui_Dialog
...
"")
        ...
        for i in range(1,len(data)-1): #len(data)-1 avoids errors on button clicks
            reports[i].clicked.connect(partial(self.display, i))#working code with number need ID now
            #print(i) 
    ...
    def retranslateUi(self, record1):
        ...
        with open('patientData.csv') as csvfile: #now modified to pull from 2nd csv file
            data = []
            reader = csv.reader(csvfile)
            for row in reader:
                data.append(row)
